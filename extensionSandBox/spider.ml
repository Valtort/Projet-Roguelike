...
and pond_oeuf pos =
  ...
  if n > 0 then begin (* Si il y a un voisin vide *)
    ...
    set egg_pos Egg;
    Queue.add ((fun () -> player (fun () -> egg egg_pos)), Egg) queuePlayer;
  end;
...
(* [appear_spider egg_pos] fait apparaitre une araignée à partir de la position [egg_pos]
   et ajoute l'araignée à la file*)
and appear_spider egg_pos =
  ...
  if n > 0 then begin
    ...
    Queue.add ((fun () -> player (fun () -> spider spider_pos)), Spider) queuePlayer
  end;
