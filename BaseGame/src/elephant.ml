...
let dir_to_camel (x, y : int * int) : int * int =
  ...
  (* [check_direction (pos_x,pos_y) (dx,dy)] 
  ...
  let rec check_direction (pos_x, pos_y : int * int) (dx, dy : int * int) : bool =
    ...
    | _     -> false (*Si il y a un cactus ou une entitée entre le chameau et l'éléphant,
    alors l'éléphant ne voit pas le chameau*)
  in
...
(** [cactus front (x,y) (dx, dy)] vérifie si il y a un cactus à la case [(x+dx,y+dy)] *)
...
(** [elephant current_position] effectue tous les prochains tours de l'éléphant à partir de la position
    [current_position] :
